...
func WalkDisk(params models.WalkDiskParams) error {
	...
	fileError := errors.New("some file error")

	if err := filepath.WalkDir(filepath.Join(params.Disk, params.Vol),
		func(path string, info os.DirEntry, err error) error {
			...
			_, errBz := bzip.Write([]byte(strings.Trim(strings.Join(row, ";"), ";") + "\n"))
			if errBz != nil {
				return fmt.Errorf("%v", errBz)
			}
...
func createFiles(path string, vol string, numberOfFiles int) ([]*bzip2.Writer, []*os.File) {
	...
	for i = 0; i < int64(numberOfFiles); i++ {
		...
		bzip, err := bzip2.NewWriter(file, &bzip2.WriterConfig{Level: bzip2.BestCompression})
		if err != nil {
			log.Println(err)
		}

		slcBzip2 = append(slcBzip2, bzip)
	}
